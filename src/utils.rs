...
pub fn string_to_ffi_buf(s: &str, buffer: *mut c_char, max_len: u64) -> Result<(), String> {
    if buffer.is_null() || max_len == 0 {
        return Err("Invalid buffer or max length".to_string());
    }

    let c_string = CString::new(s).unwrap();
    let bytes = c_string.as_bytes_with_nul();
    let bytes_to_copy = bytes.len().min(max_len as usize);

    unsafe {
        ptr::copy_nonoverlapping(bytes.as_ptr() as *const c_char, buffer, bytes_to_copy);

        if bytes_to_copy > 0 {
            *buffer.add(bytes_to_copy - 1) = 0;
        }
    }
